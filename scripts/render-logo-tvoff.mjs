...
// timeline in seconds (long hold, then collapse, dot, fade)
function state(t){
...
function frameSVG(t){
  const s=state(t);
  const bg = MODE==='alpha' ? '' : `<rect width="${W}" height="${H}" fill="${BG}"/>`;
...
let r;
if(MODE==='solid'){ r=spawnSync('ffmpeg',['-y','-loglevel','error','-framerate',String(FPS),'-i',join(tmp,'f%04d.png'),'-vf','noise=alls=11:allf=t+u','-c:v','libx264','-pix_fmt','yuv420p','-crf','17','-movflags','+faststart',`${base}.mp4`],{stdio:'inherit'}); console.log('Wrote',`${base}.mp4`); }
